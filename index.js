...
(async () =>
{
  ...
  let costInWei = new BigNumber(dropCount).times(totalCostPerDropInWei)
  ...
  for(let i = 0; i < settings.accounts.length; i++)
  {
    ...
    const tx = await scriptAccount.signTransaction({ to: account, value:dropAmountInWei, gas: 21000, gasPrice: gasPriceInWei, nonce: nonce++ })
